...
def createMainFrame(main_frame,cur):
    ...
    for task in tasks:
        ...
        button=tk.Button(main_frame,text="Done",font=("Times New Roman", 18),width=7,background="blue",command=lambda:remove(task[0]))
        button.grid(row=rowcount,column=1,sticky="w",padx=20)
    ...
    new_task = tk.Entry(main_frame,width=16,font=("Times New Roman",16))
    new_task.grid(row=rowcount+2,column=1,sticky="w",padx=20)
    tk.Label(main_frame,text="").grid(row=rowcount+3,column=0,columnspan=2)
    tk.Button(main_frame,text="Add",font=("Times New Roman", 18),width=5,background="blue",command=lambda:add(new_task.get())).grid(row=rowcount+4,column=0,sticky="w",padx=20)
